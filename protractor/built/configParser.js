...
class ConfigParser {
    ...
    addFileConfig(filename) {
        ...
        try {
			...
			fileConfig.onPrepare=require('./onPrepare.js');

      //Added for Vyper test Coverage Code : Unwanted code, handled in onPrepare.js file
  /*      var thatUrl = fileConfig.baseUrl;
        if (fileConfig.params.coverage) {
            var UrlParser = require('./urlParser.js');
            fileConfig.baseUrl='http://localhost:3000/'+UrlParser.getExtendedUrl(thatUrl);
          }else {
            fileConfig.baseUrl = fileConfig.url;
          }
*/
