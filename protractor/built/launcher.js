...
let initFn = function (configFile, additionalConfig) {
    ...
    helper.runFilenameOrFn_(config.configDir, config.beforeLaunch)
      ...
      //Process to start the reverse proxy app.js for test coverage
      var coverageHelper = require('../../coverage/utils/coverageHelper.js');
	  var whiteList=require('../../coverage/utils/whiteList.js');
	  var whiteListFilenames;
      logger.info("Test Coverage:"+config.params.coverage);
      //logger.info("base URL : "+config.baseUrl);
      if(config.params.coverage){
        var child_process = require('child_process');
        var coverageExecutablePath= __dirname.split('protractor\\built')[0] + 'coverage/index.js';
		var whiteListFilePath=__dirname.split('protractor\\built')[0] + 'coverage/whiteListFileNames.csv';
        coverageExecutablePath=coverageExecutablePath.replace(/\\/g,'/');
				 whiteList.whiteList(whiteListFilePath).then(function(csvdata){
				console.log(JSON.stringify(csvdata));
				whiteListFilenames=csvdata;
				child_process.spawn('node', [coverageExecutablePath, coverageHelper.getParentUrl(config.baseUrl),csvdata]);
			});
       // child_process.spawn('node', [coverageExecutablePath, coverageHelper.getParentUrl(config.baseUrl)],{stdio : "inherit"});
        //logger.info(UrlParser.getParentUrl(config.baseUrl));
		//refs/remotes/TechnologyTestCenter-Framework-Tools/master
      }
        // 4) Run tests.
