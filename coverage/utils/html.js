...
Report.mix(HtmlReport, {
    ...
    writeDetailPage: function (writer, node, fileCoverage) {
        ...
        var opts = this.opts,
           ...
           var sourceText = fileCoverage.code && Array.isArray(fileCoverage.code) ?
			   ...
			   fileCoverage.code.join('\n') + '\n' : sourceStore.get(path.join(scriptFolder,fileCoverage.path));
    ...
    writeFiles: function (writer, node, dir, collector) {
        ...
        node.children.forEach(function (child) {
            ...
            if (child.kind === 'dir') {
                ...
                that.writeFiles(writer, child, path.join(dir, child.relativeName), collector);
				
				
            } else {
                childFile = path.join(dir,child.relativeName + '.html');
    ...
    writeReport: function (collector, sync) {
            ...
            copyAssets = function (subdir) {
                ...
                fs.readdirSync(srcDir).forEach(function (f) {
                    var resolvedSource = path.resolve(srcDir, f),
                        resolvedDestination = path.join(dir,'/assets/', f),
		...
		scriptFolder=path.resolve(dir,'..','..','SourceFolder');
